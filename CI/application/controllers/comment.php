...
class Comment extends REST_Controller
{
   ...
   function user_comment_post()
        ...
        if (isset($status) && $status === 'OK')
            {
                ...
                // {
                    $message = "";
                    $data = $redis->lrange("comment:$aid",$offset,$limit);
    ...
    // function comment_to_string($type,$rece_id,$send_id,$aid,$comment,$date)
    // {
    //    return "$type|$rece_id|$send_id|$aid|$comment|$date";
    // }

    // function string_to_comment(&$data,$string)
    // {
    //    $data['']
    // }
}