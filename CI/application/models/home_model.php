...
class home_model extends CI_Model
{
     ...
     function user_tag_get(& $message,$limit,$offset)
     {
          $uid = $this->session->userdata('uid');
          $order = "select * from q2a_question where id in (select distinct qid from question_tag where tagid in (select tagid from user_tag where uid = $uid)) order by follow_num desc,answer_num desc,date desc limit $offset $limit";
